...
export type Provider = 'chatgpt' | 'claude' | 'perplexity' | 'abacus' | 'google';
...
export interface UserSubscriptions {
  ...
  abacus: PlanId;
  google?: PlanId;
  [provider: string]: PlanId | undefined;
}
...
export const PLAN_CATALOG: Record<PlanId, PlanDefinition> = {
  ...
  gemini_flash: {
    id: 'gemini_flash', provider: 'google', name: 'Gemini Flash', pricePerMonthUSD: 0,
    features: [ { name: 'Gemini Flash access' } ],
    limits: [ { metric: 'tokens', period: 'day', limit: 1_000_000, notes: 'Subject to account limits' } ]
  },
  gemini_pro: {
    id: 'gemini_pro', provider: 'google', name: 'Gemini Pro', pricePerMonthUSD: 20,
    features: [ { name: 'Gemini Pro models' }, { name: 'Priority usage' } ],
    limits: [ { metric: 'tokens', period: 'day', limit: 5_000_000, notes: 'Estimate; confirm with Google Cloud billing' } ]
  },
};
...
export class SubscriptionManager {
  ...
  async getSubscriptions(): Promise<UserSubscriptions> {
    const stored = (await this.store.load()) ?? DEFAULT_SUBSCRIPTIONS;
    if (!stored.google) stored.google = DEFAULT_SUBSCRIPTIONS.google;
    return stored;
  }
