...
export async function readAllMessages(): Promise<TriMessage[]> {
  try {
    ...
    return raw
      ...
      .filter(Boolean)
      .map(line => {
        try { return JSON.parse(line) as TriMessage; } catch { return null as unknown as TriMessage; }
      })
      .filter(Boolean);
  } catch {
    return [];
...
async function callClaudeWithContext(context: TriMessage[], newMsg: TriMessage): Promise<string> {
  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      resolve(`Error: Failed to write prompt to Claude stdin - ${(e as Error)?.message || e}`);
    }
  });
}
