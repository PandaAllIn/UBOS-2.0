...
export class ClaudeAgentInterface {
  ...
  constructor() {
    ...
    this.reviewAgent = new CodeReviewAgent('claude_review', 'claude_quality');
  }

  // === CODEX CLI INTEGRATION ===
  
  async executeCodexTask(task: string): Promise<string> {
  ...
  async reviewCode(files: string[], criteria?: string): Promise<string> {
    ...
    return await this.executeCodexTask(task);
  }
  ...
  async conductResearch(query: string): Promise<string> {
    ...
    const researchCost = typeof (result.metadata as Record<string, unknown> | undefined)?.researchCost === 'number'
      ? (result.metadata as Record<string, unknown>).researchCost as number
      : undefined;
    this.updateSession('Research', result.success, researchCost);
  ...
  async quickAnalysis(task: string): Promise<string> {
    const result = await this.summoner.run({ input: task, dryRun: true });
    if (!result.success) {
  ...
  async executeWorkflow(steps: Array<{
    ...
    for (const step of steps) {
      ...
      switch (step.type) {
        case 'codex':
          result = await this.executeCodexTask(step.task);
          break;
        case 'research':
          result = await this.conductResearch(step.task);
          break;
        case 'analyze':
...
export async function research(query: string): Promise<string> {
  return await claudeInterface.conductResearch(query);
}
...
export async function analyzeTask(task: string): Promise<unknown> {
