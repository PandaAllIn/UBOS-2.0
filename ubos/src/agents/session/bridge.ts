...
export class SessionBridge {
  ...
  async endSession(sessionId: string, updates: { achievement?: { xp: number; title?: string }; state?: Record<string, any> }) {
    const all = await readSessions();
    ...
    // Consolidate memory
    if (updates.state) {
      soul.memory.longTerm = { ...(soul.memory.longTerm || {}), lastSession: updates.state } as any;
