...
// Context7-guided: Enhanced Abacus agent feature definition
const enhancedAbacusFeature: AgentFeature = {
  type: 'enhanced-abacus',
  capabilities: ['research', 'analysis', 'data-retrieval', 'deep-research'],
  requirements: { supportedDomains: ['eu_funding', 'biotechnology', 'chemistry', 'market_analysis'] }
};
...
export class EnhancedAbacusAgent extends BaseAgent {
  ...
  constructor(id: string, requirementId: string) {
    super(id, requirementId, enhancedAbacusFeature);
  ...
  private formatStandardResearchOutput(result: {
    confidence: number;
    costUSD: number;
    queryId: string;
    tokensUsed: number;
...
`;
  ...
  private formatDeepResearchOutput(result: {
    primary: {
      queryId: string;
      confidence: number;
      tokensUsed?: number;
...
`;
  ...
  async getRecentResearch(limit: number = 5): Promise<unknown[]> {
