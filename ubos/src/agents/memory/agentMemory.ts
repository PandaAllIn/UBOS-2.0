...
export class AgentMemoryService {
  ...
  async consolidateMemory(agentId: string, sessionData: Record<string, any>): Promise<AgentSoul> {
    const soul = await this.loadSoul(agentId);
    // A minimal consolidation: merge keys under longTerm.insights
    soul.memory.longTerm = {
      ...soul.memory.longTerm,
      insights: {
        ...(soul.memory.longTerm?.insights || {}),
  ...
  async addSemanticMemory(agentId: string, text: string, metadata?: Record<string, any>) {
  ...
  async attestIdentity(agentId: string): Promise<{ platform: string; signature: string; timestamp: number; expiry: number; codeIntegrity: string }>{
    ...
    const base = await this.baseDirPromise;
    const filesToHash = [
