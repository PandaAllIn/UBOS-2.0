...
interface AgentSummonerMetadata {
  summoningResult?: {
    discoveredAgents?: unknown[];
    confidence?: number;
...
type SummoningPayload = NonNullable<AgentSummonerMetadata['summoningResult']>;
...
export class EnhancedMissionControl extends MissionControl {
  ...
  private async addToHistory(type: SessionMemory['conversationHistory'][0]['type'], content: string, metadata?: unknown): Promise<void> {
  ...
  async executeEnhancedTask(taskDescription: string, options: { 
    ...
    try {
      ...
      if (shouldUseSummoner) {
        ...
        const summonerMetadata = (summonerResult.metadata as AgentSummonerMetadata | undefined)?.summoningResult;

        if (summonerResult.success && summonerMetadata) {
          // Execute with discovered optimal configuration
          const optimalConfig = this.parseAgentRecommendations(summonerMetadata);
        ...
        await this.addToHistory('agent_output', `Agent Summoner: ${summonerResult.output}`, {
          cost: summonerMetadata?.estimatedCost,
          confidence: summonerMetadata?.confidence
        });
  ...
  private parseAgentRecommendations(summoningResult: SummoningPayload): OptimalConfig {
  ...
  private async executeWithOptimalConfig(
    ...
    const existingMetadata = (enhancedOptions.metadata as Record<string, unknown> | undefined) ?? {};
    enhancedOptions.metadata = {
      ...existingMetadata,
      agentRecommendations: config.preferredAgents,
      complexity: config.estimatedComplexity,
      approach: config.recommendedApproach,
      budget: config.budgetConstraints,
    };

    return await this.orchestrator.execute(taskDescription, enhancedOptions);
  }

  private async updateSessionContext(taskDescription: string, result: OrchestrationResult): Promise<void> {
    // Update session memory with task completion
    if (!this.sessionMemory.context.completedTasks) {
      this.sessionMemory.context.completedTasks = [] as unknown[];
    }
    ...
    const additionalCost = result.results.reduce((total, agentResult) => total + (agentResult.costUSD ?? 0), 0);
    if (additionalCost > 0) {
      this.sessionMemory.context.researchCosts =
        (this.sessionMemory.context.researchCosts || 0) + additionalCost;
    }
