...
export class MissionControl {
  ...
  async getStatus(): Promise<MissionControlStatus> {
    ...
    return {
      ...
      system: {
        agents: agentStats,
        tools: { available: 8, errors: 0 }, // CodexAgent, CodeReviewAgent, etc.
  ...
  private calculateAgentStatsFromActions(actions: { status: string }[]) {
  ...
  private calculateTodayCost(usage: { ts: string, costUSD?: number }[]): number {
    const today = new Date().toISOString().split('T')[0];
    return usage
