...
import { NotionSyncAgent } from '../agents/premium/notionSyncAgent.js';
const notionSyncAgent = new NotionSyncAgent();
	...
	const cmd = process.argv[2] || 'help';
        ...
        case 'notion:sync-projects': {
            try {
                ...
                console.error('❌ Notion projects sync failed:', (e as Error).message);
                process.exit(1);
            }
            break;
        }
        ...
        case 'notion:sync-agents': {
            try {
                ...
                console.error('❌ Notion agents sync failed:', (e as Error).message);
                process.exit(1);
            }
            break;
        }
        ...
        case 'notion:sync-funding': {
            try {
                ...
                console.error('❌ Notion funding sync failed:', (e as Error).message);
                process.exit(1);
            }
            break;
        }
        ...
        case 'notion:daily-update': {
            try {
                ...
                console.error('❌ Notion daily update failed:', (e as Error).message);
                process.exit(1);
            }
            break;
        }
        ...
        case 'notion:start-scheduler': {
            try {
                ...
                console.error('❌ Scheduler start failed:', (e as Error).message);
                process.exit(1);
            }
            break;
        }
        ...
        case 'notion:stop-scheduler': {
            try {
                ...
                console.error('❌ Scheduler stop failed:', (e as Error).message);
                process.exit(1);
            }
            break;
        }
        ...
        case 'spec:list': {
            try {
                ...
                const entries = Object.entries(reg) as Array<[string, { version: string; updatedAt: string }]>;
                ...
                console.error('❌ spec:list failed:', (e as Error).message);
                process.exit(1);
            }
            break;
        }
        ...
        case 'spec:events': {
            try {
                ...
                const raw = await fs.readFile(p, 'utf-8').catch(() => '');
                const events: { ts: string; traceId: string; id: string; version: string; services: string[] }[] = raw ? JSON.parse(raw) : [];
                ...
                console.error('❌ spec:events failed:', (e as Error).message);
                process.exit(1);
            }
            break;
        }
        ...
        case 'spec:diff': {
            try {
                ...
                const raw = await fs.readFile(p, 'utf-8').catch(() => '');
                const changes: { path: string, from: string, to: string, diff: { added: number, removed: number, same: number } }[] = raw ? JSON.parse(raw) : [];
                ...
                console.error('❌ spec:diff failed:', (e as Error).message);
                process.exit(1);
            }
            break;
        }
			...
			break;
				...
				case 'tri:chat': {
					try {
						...
						console.error('Chat UI error:', (error as Error).message);
					}
					break;
				}
				...
				case 'smoke:basic': {
					...
					try {
						...
						const test = factory.create({ id: 'smoke_test', type: 'SmokeTestAgent', requirementId: 'smoke', capabilities: [], params: {} });
...
📋 ${opp.title}`);
				...
				try {
						...
						console.error('Perplexity smoke failed:', (e as Error).message);
						process.exit(1);
					}
					break;
				}
				...
				case 'codex:status': {
					try {
						...
						console.error('Error checking Codex:', (error as Error).message);
					}
					break;
				}
				...
				try {
						...
						console.error('❌ Session update failed:', (error as Error).message);
					}
					break;
				}
				case 'claude:status': {
				...
				case 'system:analyze': {
					try {
						...
						console.error('❌ System analysis error:', (error as Error).message);
						process.exit(1);
					}
					break;
				}
