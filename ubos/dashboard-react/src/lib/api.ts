export async function getJSON<T = unknown>(path: string, opts: { timeoutMs?: number } = {}): Promise<T> {
...
export function wsURL(): string {
  ...
  const isDev = import.meta && (import.meta as { env: Record<string, unknown> }).env && (import.meta as { env: Record<string, unknown> }).env.DEV
  const targetPort = isDev ? '3000' : (port || (protocol === 'https:' ? '443' : '80'))
  return `${scheme}://${hostname}:${targetPort}`
}

export async function postJSON<T = unknown>(path: string, body: unknown, opts: { timeoutMs?: number } = {}): Promise<T> {
  const controller = new AbortController()
