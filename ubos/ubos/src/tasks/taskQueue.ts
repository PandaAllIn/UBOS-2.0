...
async function readQueue(): Promise<QueueState> {
  ...
  catch (err: unknown) { if ((err as NodeJS.ErrnoException).code === 'ENOENT') return { tasks: [], updatedAt: Date.now() }; throw err; }
}
...
export async function processOne(kernel: UBOSKernel): Promise<{ processed: boolean; task?: unknown; result?: unknown }>{
  const item = await dequeue();
  if (!item) return { processed: false };
  const factory = new AgentFactory(kernel);
  factory.registerAgentType('BusinessDevelopment', BusinessDevelopmentAgent as unknown as new (kernel: UBOSKernel, soul: unknown) => BaseAgent);
  factory.registerAgentType('ProposalWriter', ProposalWriterAgent as unknown as new (kernel: UBOSKernel, soul: unknown) => BaseAgent);
  factory.registerAgentType('PartnerMatcher', PartnerMatcherAgent as unknown as new (kernel: UBOSKernel, soul: unknown) => BaseAgent);
