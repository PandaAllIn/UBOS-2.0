...
import { hasGenesisBlock } from '../memory/repo';
...
export class UBOSKernel {
  ...
  private territories: Map<string, unknown> = new Map();
  ...
  async boot(): Promise<void> {
    ...
    try {
      ...
      console.log(`⚙️ Spec Interpreter initialized for constitution v${(constitutionSpec.metadata as { version: string }).version}`);
  ...
  private extractInterfaces(markdown: string): unknown[] {
  ...
  private async initializeTerritories(traceId?: string): Promise<void> {
    ...
    if (loaded.size > 0) {
      for (const [key, territory] of loaded) {
        await (territory as { initialize?: () => Promise<void> }).initialize?.();
  ...
  public async loadTerritorySpecs(traceId?: string): Promise<Map<string, unknown>> {
    ...
    try {
      ...
      for (const f of files) {
        ...
        try {
          let events: { ts: string; traceId: string | undefined; file: string; id: string; version: unknown; services: string[] }[] = [];
  ...
  getTerritory(key: string): unknown | undefined {
