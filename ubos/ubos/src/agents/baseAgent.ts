...
export abstract class BaseAgent {
  ...
  constructor(protected kernel: UBOSKernel, soul: AgentSoul) {
    ...
    this.state = {
      id: soul.agentId,
      type: (soul as { type: string }).type || 'AGENT',
      soul,
      status: 'idle',
      memory: {},
      credits: 0,
  ...
  async remember(key: string, value: unknown): Promise<void> {
    ...
    soul.memory.longTerm = {
      ...(soul.memory.longTerm || {}),
      agentMemory: { ...((soul.memory.longTerm as { agentMemory?: Record<string, unknown> })?.agentMemory || {}), [key]: value },
    };
    await this.memory.saveSoul(soul);
  }
  ...
  async recall(key: string): Promise<unknown> {
    const soul = await this.memory.loadSoul(this.state.id);
    return (soul.memory.longTerm as { agentMemory?: Record<string, unknown> })?.agentMemory?.[key];
  }

  // Collaboration stubs
  async requestHelp(fromAgent: string, task: Task): Promise<unknown> {
    return { requestedFrom: fromAgent, task };
  }

  async delegateTask(toAgent: string, task: Task): Promise<unknown> {
