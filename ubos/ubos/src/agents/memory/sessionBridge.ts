...
export class SessionBridgeProtocol {
  ...
  async initializeSession(platform: string, agentId: string, context: Partial<SessionContext['context']>): Promise<SessionContext> {
    ...
    this.currentSession = {
      sessionId,
      agentId,
      platform: platform as 'claude-code' | 'cursor' | 'vscode' | 'terminal' | 'web',
      startTime: Date.now(),
  ...
  async persistSessionMemory(agentId: string, sessionData: unknown): Promise<void> {
    const enrichedData = {
      ...sessionData,
...
Current capabilities: ${this.extractCapabilities(soul).join(', ')}`;
  ...
  private extractCapabilities(soul: { achievements: { titles: string[] } }): string[] {
  ...
  private extractCurrentTasks(soul: { memory: { shortTerm?: { currentTasks?: unknown[] } } }): unknown[] {
  ...
  private extractCollaborators(soul: { memory: { longTerm?: { collaborators?: string[] } } }): string[] {
