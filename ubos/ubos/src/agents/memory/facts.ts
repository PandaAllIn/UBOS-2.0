...
async function readFacts(): Promise<FactState> {
  try {
    ...
    if ((err as NodeJS.ErrnoException).code === 'ENOENT') return { facts: [], updatedAt: Date.now() };
    throw err;
  }
}
...
export function extractFactsFromSession(agentId: string, sessionData: Record<string, unknown>): Array<Omit<Fact, 'id' | 'timestamp'>> {
