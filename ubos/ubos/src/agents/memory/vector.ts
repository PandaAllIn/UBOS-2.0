...
async function readVectors(): Promise<VectorState> {
  try {
    ...
    if ((err as NodeJS.ErrnoException).code === 'ENOENT') return { entries: [], updatedAt: Date.now() };
    throw err;
  }
}
...
export class LocalVectorStore {
  ...
  async add(agentId: string, text: string, metadata?: Record<string, unknown>): Promise<string> {
  ...
  async search(agentId: string, query: string, k = 5): Promise<Array<{ id: string; score: number; text: string; metadata?: unknown }>> {
    const state = await readVectors();
