...
type AgentCtor = new (kernel: UBOSKernel, soul: unknown) => BaseAgent;
...
async function readAgents(): Promise<AgentsState> {
  ...
  catch (err: unknown) { if ((err as NodeJS.ErrnoException).code === 'ENOENT') return { agents: [] }; throw err; }
}
...
export class AgentFactory {
  ...
  async createAgent(type: string, config: AgentConfig): Promise<BaseAgent> {
    ...
    const soul = await mem.loadSoul(soulId).catch(() => ({ agentId: soulId, type } as unknown));
    const agent = new Ctor(this.kernel, soul as unknown);
  ...
  async spawnAgent(type: string, soulId?: string): Promise<BaseAgent> {
    ...
    try {
      const mem = new AgentMemoryService();
      const soul = await mem.loadSoul((agent as { state: { id: string } }).state.id);
  ...
  async getAgent(id: string): Promise<BaseAgent | null> {
    ...
    if (!Ctor) return null;
    const mem = new AgentMemoryService();
    const soul = await mem.loadSoul(rec.soulId).catch(() => ({ agentId: rec.soulId, type } as unknown));
    const agent = new Ctor(this.kernel, soul as unknown);
  ...
  async findCapableAgent(capability: string): Promise<BaseAgent | null> {
    ...
    for (const a of this.instances.values()) {
      const caps = (a as { capabilities?: { canExecute?: string[] } }).capabilities?.canExecute || [];
