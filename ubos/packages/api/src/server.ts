...
import { apiBillingMiddleware } from '../../src/middleware/api-billing.js';
...
export async function buildServer() {
  try { (await import('./tracing.js')).initTracing(); } catch (e) { console.error(e) }
  ...
  await app.register(fp(apiBillingMiddleware));
  const { eufmPlugin } = await import('./eufm.js');
  await app.register(eufmPlugin);
  ...
  app.addHook('onRequest', async (req) => {
    if (req.ctx?.tenantId) {
      // @ts-expect-error fastify logger child available
      (req as { log: { child: (bindings: Record<string, unknown>) => unknown } }).log = req.log.child({ tenantId: req.ctx.tenantId });
    }
  });
  ...
  app.post('/v1/orchestrate', async (req, reply) => {
    ...
    if (req.ctx.tenantId) await recordRequest({ id: requestId, tenantId: req.ctx.tenantId, provider: (adapter as { name: string }).name, latencyMs: res.latencyMs, inputTokens: res.inputTokens, outputTokens: res.outputTokens, costUsd: res.costUsd });
  ...
  app.get('/v1/tasks/:id', async (req, reply) => {
    ...
    const task = await getTask((req.params as { id: string }).id);
  ...
  app.get('/v1/metrics/usage', async (req, reply) => {
    ...
    const unit = (q.unit) || 'requests';
    const { usageAggregate } = await import('./lib/metrics.js');
    if (!req.ctx.tenantId) { reply.code(400).send({ error: 'X-Tenant-Id header is required' }); return; }
    const agg = await usageAggregate(req.ctx.tenantId, q.from, q.to, unit as 'requests'|'tokens'|'cost_usd');
    reply.send(agg);
  });
