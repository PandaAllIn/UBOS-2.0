...
const connection = process.env.REDIS_URL ? new IORedis(process.env.REDIS_URL) : undefined as unknown;
export const queue = connection ? new Queue('orchestrate', { connection }) : undefined;

export function enabled(): boolean { return !!queue; }

export async function enqueueRedis(name: string, data: unknown, opts?: JobsOptions) {
...
export function startRedisWorker() {
  ...
  const worker = new Worker('orchestrate', async () => {
